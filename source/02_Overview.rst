...
| GELATOでは2次元の「流れ」に乗ったトレーサーの追跡をラグランジェ的に行うので、「流れ」の計算結果は予め用意しておく必要がある。
| Nays2DH、Nays2DFlood、Nays2D+、FastMechなど2次元ソルバーのうち、流速および水深、河床高出力するソルバーの計算結果計(CGNSファイル)を読み込む形式となっている。( :numref:`flow` )。
...
GELATOで使用する流れの計算結果が格納されたCGNSファイルはソルバー起動時に表示されるダイアログから指定する。
...
| GELATOではほとんどの場合、計算格子についても前記の計算結果GNSファイルに含まれるものからインポートして使用する。
| 前述のダイアログで流れの計算結果のCGNSファイルを指定すると、:numref:`import_grid` に示すダイアログが表示され、「はい」を選択すると自動で計算格子が読み込まれる。
...
| GELATOから流れの計算プロジェクトのCGNSファイルに含まれる格子データを読もうをすると、:numref:`import_grid_2` のような警告が出る。
| これは、現在起動中のプロジェクトがGELATOであるにも関わらず、別のプロジェクト（流れの計算プロジェクト）の格子ファイルをインポートしようとしていることに対する警告であるが、構わず「OK」を押して次へ進むと、:numref:`import_grid_3` のような対象の流れの計算結果から格子がインポートされ、その結果が表示される。
...
| ここではGELATOの計算に必要となる計算条件について述べる。   
